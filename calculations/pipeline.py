...
from calculations import capital_structure, cashflow_quality, dilution, growth, margins, scoring
...
def compute_year_metrics(corp_code: str, fiscal_year: int, reprt_code: str = "11011") -> dict:
    row = {
        ...
        "fs_div_used": None,
        "shares_outstanding": dilution.get_shares_for_year(corp_code, fiscal_year, reprt_code),
    }

    bundle = resolve_statement(corp_code, str(fiscal_year), reprt_code)
    row["fs_div_used"] = bundle.fs_div_used
    if not bundle.data:
        return row
    ...
    row.update(margins.compute(row))
    return row
...
def run_for_company(corp_code: str, years: list[int], reprt_code: str = "11011") -> list[dict]:
    ...
    rows = growth.attach_growth_metrics(rows)
    rows = margins.attach_margin_changes(rows)
    rows = dilution.attach_share_changes(rows)
    rows = scoring.attach_scores(rows)
    return rows
